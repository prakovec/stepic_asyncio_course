...
async def check_message(message):
    ...
    role = current_task.get_name()
    message_temp = message['message'].lower().translate(str.maketrans('', '', '.,!?;:'))
    match role:
        case "admin":
        ...
        case "moderator":
            for word in banned_words:
                if word in message_temp:
                    mes = message['message'].replace(word, "****")
                    break
                else:
                    mes = message['message']
            print(f"moderator: {mes}")
        case "student":
            for word in banned_words:
                if word in message_temp:
                    print("student: В сообщении есть запрещённое слово, сообщение скрыто")
                    return
...
async def main():
    ...
    for message in messages:
        tasks.append(asyncio.create_task(check_message(message), name=str(message['role'])))
